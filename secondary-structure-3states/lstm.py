...
with graph.as_default():
  ...
  rand = random.sample(range(930), 50)
  '''
  output_list = []
  for step_index in range(930):
    if step_index in rand:
      output_list.append(tf.stop_gradient(outputs[step_index, :, :]))
    else:
      output_list.append(outputs[step_index, :, :])
  outputs = tf.stack(output_list)
  del output_list
  '''

  for step_index in range(0, 930):
    if step_index == 0:
      output_final = tf.concat([outputs[step_index, :, :]], 0)
    elif step_index in rand:
      output_final = tf.concat([output_final, tf.stop_gradient(outputs[step_index])], 0)
    else:
      output_final = tf.concat([output_final, outputs[step_index]], 0)

  structure = tf.reshape(output_final, [-1, nb_lstm_inside])
