...
with tf.Session(graph=graph) as session:
  ...
  train_weight_mask_ss = np.asarray(weight_mask_ss[20 * len(data) / 100 :])
    ...
    for i in range(100):
      ...
      for j in range(len(test_data)):
        ...
        for k in range(test_seq_len[j]):
          label_tmp.append(np.random.choice(np.arange(0, 8), p=[0.3313, 0.0372, 0.0105, 0.0052, 0.2329, 0.1079, 0.0794, 0.1956]))
